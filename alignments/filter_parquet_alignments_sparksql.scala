...
val logger = LoggerFactory.getLogger("filter_parquet_alignments_sparksql")
...
println(spark.sql("select count(*) from alignments a where a.start < 10").first.getLong(0))
